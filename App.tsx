  ...
  const handleSaveToDb = async () => {
    ...
    try {
        ...
        if (isSupabaseConfigured() && supabase && session.id !== 'admin-local') {
            ...
            if (error) { console.error("Supabase save error:", error); alert("Errore salvataggio database: " + error.message); } else { alert(editingPageId ? "Pagina aggiornata con successo!" : "Pagina pubblicata con successo!"); fetchPublicPages(); handleCloseEditor(); }
        } else {
